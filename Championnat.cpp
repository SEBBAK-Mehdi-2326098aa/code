...
// je déclare mon tableau qui reçois les noms des joueurs et le vecteurs associés qui reçoit les points de chaque équipe
    ...
    for ( size_t i = 0; i<joueurs.size(); i++ ){
        ...
        int premierround = duel(joueurs[i], joueurs[j]);

        if (premierround == 1) {
            pointsjoueurs[i]++;


            cout << joueurs[i] << " a gagné" << endl << endl;        }
        else {

            pointsjoueurs[j]++;


            cout << joueurs[i+1] << " a gagné" << endl << endl;
        }
        }
    }
        ...
        cout << joueurs[0] << " sont les grands gagnants du tournoi ";
